    ...
    <main class="min-h-screen grid grid-cols-1 md:grid-cols-[1.1fr_1fr]">
        ...
        <section class="flex items-center justify-center p-8 md:p-10 bg-white relative z-20">
            <article class="w-full max-w-[360px]">
                ...
                <header class="mb-6">
                    <h2 class="font-serif text-[26px] font-bold text-[#17324a] mb-1">Selamat Datang!</h2>
                    <p class="text-[#5a7280] text-[14.5px]">Daftar untuk kelola keuangan UMKM kamu</p>
                </header>

                <!-- Tabs (Daftar Aktif) -->
                <nav class="flex bg-[#eaf6fc] rounded-full p-1 mb-7" aria-label="Navigasi Autentikasi">
                    <a href="{{ route('login') }}" class="flex-1 text-center text-[#5a7280] hover:text-[#17324a] font-semibold rounded-full py-2.5 text-[14.5px] transition-all">
                        Log in
                    </a>
                    <a href="{{ route('register') }}" class="flex-1 text-center bg-white text-[#17324a] shadow-[0_6px_14px_-8px_rgba(23,50,74,0.35)] font-semibold rounded-full py-2.5 text-[14.5px] transition-all">
                        Daftar
                    </a>
                </nav>

                @if($errors->any())
                    <aside class="bg-[#fbe4e4] text-[#e15c5c] p-4 rounded-[10px] mb-5 text-[12.5px] font-medium" role="alert">
                ...
                <form action="{{ route('register') }}" method="POST">
                    ...
                    <fieldset class="border-none p-0 m-0 space-y-4">
                        ...
                        <div class="relative">
                            <label for="store_name" class="sr-only">Nama Toko</label>
                            <input type="text" id="store_name" name="store_name" placeholder="Nama Toko" 
                                class="w-full px-5 py-[13px] rounded-full border border-[#e2edf3] bg-[#eaf6fc] text-[14.5px] text-[#17324a] placeholder-[#93a7b3] focus:outline-none focus:border-[#4bc3e0] focus:bg-white transition-colors">
                        </div>
                        ...
                        <div class="relative">
                            ...
                            <input type="password" id="password" name="password" placeholder="Kata Sandi (Min. 8 Karakter)" 
                                class="w-full px-5 py-[13px] rounded-full border border-[#e2edf3] bg-[#eaf6fc] text-[14.5px] text-[#17324a] placeholder-[#93a7b3] focus:outline-none focus:border-[#4bc3e0] focus:bg-white transition-colors" required>
                        </div>
                    </fieldset>
                    
                    <button type="submit" class="w-full bg-gradient-to-br from-[#4bc3e0] to-[#2e9bc7] text-white font-semibold py-3.5 rounded-full shadow-[0_10px_20px_-8px_rgba(46,155,199,0.55)] hover:shadow-[0_14px_24px_-8px_rgba(46,155,199,0.65)] hover:-translate-y-[1px] transition-all text-[15px] mt-6">
                        Daftar Sekarang
                    </button>
                </form>
                
                <footer class="text-center mt-6">
                    <p class="text-[14px] text-[#5a7280]">Sudah punya akun? <a href="{{ route('login') }}" class="text-[#1e6f8c] font-bold hover:underline">Masuk!</a></p>
                </footer>
            </article>
        </section>
